...
${"d_app_vicini".$num1} = esegui_query("select app_vicini from $tableappartamenti where idappartamenti = '$idappartamenti' ");
...
$app_esiste = esegui_query("select * from $tableappartamenti where idappartamenti = '$n_nome_app'");
$num_app_esiste = numlin_query($app_esiste);
if ($num_app_esiste != 0) {
...
$messaggi = "";
if (strcmp($n_nome_app,$comp_nome_app)) $messaggi .= mex("Il nome dell'appartamento verrà cambiato da",'unit.php')." $idappartamenti ".mex("a",$pag)." $n_nome_app.<br>";
...
if ($n_app_vicini != $comp_app_vicini) $messaggi .= mex("Gli appartamenti vicini verranno cambiati",'unit.php').".<br>";
if ($n_commento != $comp_commento) $messaggi .= mex("Il commento verrà cambiato",$pag).".<br>";
if ($messaggi) echo mex("Appartamento",'unit.php')." $idappartamenti:<br>$messaggi<br>";
...
esegui_query("update $tableappartamenti set app_vicini = '".aggslashdb($n_app_vicini)."' where idappartamenti = '$idappartamenti' ");
} # fine if ($n_app_vicini != $comp_app_vicini)

if ($n_numcasa != $comp_numcasa) {
$modificato = "SI";
esegui_query("update $tableappartamenti set numcasa = '$n_numcasa' where idappartamenti = '$idappartamenti' ");
} # fine if ($n_numcasa != $comp_)
if ($n_numpiano != $comp_numpiano) {
$modificato = "SI";
esegui_query("update $tableappartamenti set numpiano = '$n_numpiano' where idappartamenti = '$idappartamenti' ");
...
else $n_maxoccupanti = "'$n_maxoccupanti'";
esegui_query("update $tableappartamenti set maxoccupanti = $n_maxoccupanti where idappartamenti = '$idappartamenti' ");
...
else $n_priorita = "'$n_priorita'";
esegui_query("update $tableappartamenti set priorita = $n_priorita where idappartamenti = '$idappartamenti' ");
} # fine if ($n_priorita != $comp_priorita)
if ($n_commento != $comp_commento) {
$modificato = "SI";
$n_commento = aggslashdb($n_commento);
esegui_query("update $tableappartamenti set commento = '$n_commento' where idappartamenti = '$idappartamenti' ");
} # fine if ($n_commento != $comp_commento)
if ($modificato == "SI") echo mex("L'appartamento",'unit.php')." $idappartamenti ".mex("è stato modificato",'unit.php').".<br>";
} # fine for $num1
...
echo "<h3>".mex("Modifica l'appartamento",'unit.php')." $idappartamenti.</h3>";
...
<tr><td>".mex("Capienza",$pag).": <b>$d_maxoccupanti</b> ".mex("Persone",$pag)."</td>
<td>".mex("Cambia in",$pag)." <input type=\"text\" name=\"n_maxoccupanti0\" size=\"2\" maxlength=\"2\"></td></tr>
<tr><td>".mex("Priorità",$pag).": <b>$d_priorita</b></td>
<td>".mex("Cambia in",$pag)." <input type=\"text\" name=\"n_priorita0\" size=\"5\" maxlength=\"5\"></td></tr>
<tr><td>".mex("Appartamenti vicini",'unit.php').": </td>
<td> <input type=\"text\" name=\"n_app_vicini0\" size=\"25\" value=\"$d_app_vicini\"> (".mex("lista di appartamenti separati da virgole",'unit.php').")</td></tr>
</table>
".mex("Commento",$pag).":<br>
<textarea name=\"n_commento0\" rows=3 cols=60 style=\"white-space: pre; overflow: auto;\">$d_commento</textarea><br>
<div style=\"text-align: center;\"><br>
".mex("<b>Attenzione</b>: le prenotazioni già inserite in questo appartamento <b>non</b> verranno spostate",'unit.php').",<br>
".mex("anche se le loro caratteristiche non sono più compatibili",$pag).".<br>
<input class=\"sbutton\" id=\"modi\" type=\"submit\" name=\"modificaappartamento\" value=\"".mex("Modifica l'appartamento",'unit.php')." $idappartamenti\">
...
".ucfirst(mex("descrizione dell'appartamento",'unit.php'))." <em>$idappartamenti</em>:<br>
...
echo "".ucfirst(mex("foto dell'appartamento",'unit.php'))." <em>".$idappartamenti."</em>:<br>";
...
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"idappartamenti\" value=\"$idappartamenti\">
<input class=\"sbutton\" id=\"canc\" type=\"submit\" name=\"cancella_app\" value=\"".mex("Cancella l'appartamento",'unit.php')." $idappartamenti\">
</div><br></form></div>
