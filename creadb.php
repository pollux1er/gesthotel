...
if (!$numappartamenti or controlla_num_pos($numappartamenti) == "NO") $numappartamenti = 5;
...
if ($HOTELD_DB_TYPE) $tipo_db = $HOTELD_DB_TYPE;
...
esegui_query("create table $tableappartamenti ( idappartamenti varchar(100) primary key, numpiano text, maxoccupanti integer, numcasa text, app_vicini text, priorita integer, commento text )");
...
esegui_query("create table $tablesessioni (idsessioni varchar(30) primary key, idutente integer, indirizzo_ip text, user_agent text, ultimo_accesso $DATETIME)");
...
<form accept-charset=\"utf-8\" method=\"post\" action=\"creadb.php?numappartamenti=$numappartamenti\"><div>
<hr style=\"width: 95%\">";
$zeri = (string) "0000000000000000000000000000";
for ( $num = 1; $num <= $numappartamenti; $num = $num + 1) {
$numapp = "numapp" . $num;
$piano = "piano" . $num;
...
$num_default = (string) substr($zeri,0,(strlen($numappartamenti) - strlen($num))).$num;
echo "
$num). ".mex2("Numero (o nome) dell' appartamento",'unit.php',$lingua).": 
<input type=\"text\" name=\"$numapp\" size=\"5\" value=\"$num_default\">
 ".mex2("Massimo numero di occupanti",$pag,$lingua).": 
<input type=\"text\" name=\"$maxoccupanti\" size=\"3\"><br>
".mex2("Numero (o nome) piano",$pag,$lingua).": <input type=\"text\" name=\"$piano\" size=\"4\">
...
echo "<br>".mex2("I dati inseriti non sono esatti o il database non è in ascolto (se postgres assicurarsi che venga avviato con -i e di avere i permessi giusti in pg_hba.conf)",$pag,$lingua).".<br>";
$torna_indietro = "SI";
...
$numapp = "numapp" . $num;
$numapp = $$numapp;
if (C_UTILIZZA_SEMPRE_DEFAULTS == "SI") $numapp = (string) substr($zeri,0,(strlen($numappartamenti) - strlen($num))).$num;
if (C_UTILIZZA_SEMPRE_DEFAULTS == "AUTO") $numapp = (string) substr($zeri,0,(max(strlen($numappartamenti),2) - strlen($num))).$num;
...
<h4>".mex("Benvenuto a HotelDruid!",$pag)."</h4><br>
".mex("Questi sono alcuni semplici passi che puoi seguire per configurare le funzionalità di base di HotelDruid",$pag).":<br>
...
<input type=\"text\" name=\"port_phprdb\" value=\"".C_CREADB_PORT."\">(".mex2("Normalmete 5432 o 5433 per Postgresql o 3306 per Mysql",$pag,$lingua).")<br>
...
<input type=\"text\" name=\"numappartamenti\"><br>
