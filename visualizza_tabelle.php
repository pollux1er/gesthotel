...
$tasse = ($costo_agg_parziale / ($dati_cap[$numca]['tasseperc'] + 100)) * $dati_cap[$numca]['tasseperc'];
$tasse = $tasse / $dati_tariffe['tasse_arrotond'];
$tasse = round($tasse);
$tasse = $tasse * $dati_tariffe['tasse_arrotond'];
$tasse_TOT = $tasse_TOT + $tasse;
...
echo "</tr></table><div style=\"text-align: center;\">* ".mex("entrata presa dalle prenotazioni",$pag)."$stringa_pagine
<br>";
...
<td>".mex("Inizio_$parola_settimana",$pag)."</td>
<td>".mex("Fine_$parola_settimana",$pag)."</td>";
...
$riga_prezzi_r .= "<td><input type=\"text\" name=\"pertar$numtariffa\" value=\"$tariffa\" size=\"8\">";
...
if ($dati_ca[$num1]['tasseperc']) $prezzo_p .= "<br><small>(".strtolower(mex("Tasse",$pag))."&nbsp;".$dati_ca[$num1]['tasseperc']."%)</small>";
...
if ($tariffa_cerca == $tariffa_per_app) $tariffa_vedi = mex("tariffa","prenota.php").$numtariffa;
} # fine for $numtariffa
echo mex("Sei sicuro di voler <b>cancellare</b> la regola di assegnazione 2 per la",$pag)." $tariffa_vedi?<br>";
...
for ($numtariffa = 1 ; $numtariffa <= $numero_tariffe ; $numtariffa = $numtariffa + 1) {
$tariffa_cerca = "tariffa".$numtariffa;
if ($tariffa_cerca == $tariffa_per_utente) $tariffa_vedi = mex("tariffa","prenota.php").$numtariffa;
} # fine for $numtariffa
echo mex("Sei sicuro di voler <b>cancellare</b> la regola di assegnazione 3 per la",$pag)." $tariffa_vedi?<br>";
...
for ($numtariffa = 1 ; $numtariffa <= $numero_tariffe ; $numtariffa = $numtariffa + 1) {
$tariffa_cerca = "tariffa".$numtariffa;
if ($tariffa_cerca == $tariffa_per_persone) $tariffa_vedi = mex("tariffa","prenota.php").$numtariffa;
} # fine for $numtariffa
echo mex("Sei sicuro di voler <b>cancellare</b> la regola di assegnazione 4 per la",$pag)." $tariffa_vedi?<br>";
...
unlock_tabelle($tabelle_lock);
} # fine if ($idcosti and !$torna_alla_tab and $id_utente == 1)
...
$regole = esegui_query("select * from $tableregole where app_agenzia != '' order by app_agenzia");
...
$n_app = str_replace (",","",$n_app);
if (str_replace (" ","",$n_app) == "") $n_app = str_replace (" ","_",$n_app);
$n_app = elimina_caratteri_slash($n_app);
...
if ($crea_app != "SI") {
if ($inserire != "NO") {
echo mex("L'appartamento",'unit.php')." <b>$n_app</b> ".mex("verrà aggiunto",'unit.php').".<br>
<form accept-charset=\"utf-8\" method=\"post\" action=\"visualizza_tabelle.php\"><div>
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"n_app\" value=\"$n_app\">
...
if ($inserire != "NO") {
esegui_query("insert into $tableappartamenti (idappartamenti) values ('$n_app') ");
...
else $maxoccupanti = $maxoccupanti ." ".mex("persone",$pag);
...
<td><input type=\"text\" name=\"n_maxoccupanti$num1\" value=\"$maxoccupanti\" size=\"8\"></td>
...
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"tipo_tabella\" value=\"appartamenti\">
<input class=\"sbutton\" type=\"submit\" name=\"crea_app\" value=\"".mex("Crea un nuovo appartamento",'unit.php')."\">
...
if ($id_utente == 1 and $num_tutti_utenti > 1) echo "<td>".mex("Utente",$pag)."</td>";
if (!$modifica_tab_beni_inv) echo "<td>".mex("Modifica",$pag)."</td>";
echo "</tr>";
...
$id_prenota = risul_query($tutte_prenota,$num1,"$tableprenota_stat.idprenota");
$id_data_inizio = risul_query($tutte_prenota,$num1,"$tableprenota_stat.iddatainizio");
$id_data_fine = risul_query($tutte_prenota,$num1,"$tableprenota_stat.iddatafine");
$num_sett = $id_data_fine - $id_data_inizio + 1;
$data_inserimento = risul_query($tutte_prenota,$num1,"$tableprenota_stat.datainserimento");
$tariffa = explode("#@&",risul_query($tutte_prenota,$num1,"$tableprenota_stat.tariffa"));
$sconto = risul_query($tutte_prenota,$num1,"$tableprenota_stat.sconto");
if ((double) $sconto > (double) $tariffa[1]) $sconto = $tariffa[1];
$prezzo_tariffa = (double) $tariffa[1];
$tariffa_tot = risul_query($tutte_prenota,$num1,"$tableprenota_stat.tariffa_tot");
$tariffe_settimanali_int = risul_query($tutte_prenota,$num1,"$tableprenota_stat.tariffesettimanali");
...
$commissioni = (double) risul_query($tutte_prenota,$num1,"$tableprenota_stat.commissioni");
if ($commissioni > 0) {
...
if (!$prezzi['tasse']) {
$tasse_tot = 0;
$tasse_perc = (double) risul_query($tutte_prenota,$num1,"$tableprenota_stat.tasseperc");
$caparra = (double) risul_query($tutte_prenota,$num1,"$tableprenota_stat.caparra");
$numpersone = risul_query($tutte_prenota,$num1,"$tableprenota_stat.num_persone");
if ($tasse_perc) {
...
$origine = risul_query($tutte_prenota,$num1,"$tableprenota_stat.origine");
$nazionalita = risul_query($tutte_prenota,$num1,"$tableclienti.nazionalita");
